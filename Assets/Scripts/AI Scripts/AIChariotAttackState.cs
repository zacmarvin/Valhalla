...
public class AIChariotAttackState : AIStateController
{
    ...
    public override void FixedUpdateState(AIController ai)
    {
        if(ai.target != null)
        {
            ...
            if (runaway)
            {
                if (Time.time - ai.timeSinceLastAttackAction > ai.minAttackWaitTime)
                {
                ...
                if (ai.chariotDistanceToRunaway < (ai.transform.position - ai.target.transform.position).sqrMagnitude)
                {
                    runaway = false;
                }
                else if (Time.time - ai.timeSinceLastAttackAction > ai.maxAttackWaitTime)
                {
                    runaway = false;
                }
            }
